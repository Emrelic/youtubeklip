...
function downloadVideo(url) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      await ytdlp(["-f", "best[ext=mp4]/best", "--no-playlist", "-o", tempPath, url]);
...
function cutSegment(inputPath, start, duration, outputPath) {
  return new Promise((resolve, reject) => {
    const proc = spawn(ffmpegPath, [
      "-i", inputPath, "-ss", String(start), "-t", String(duration),
      "-c:v", "libx264", "-c:a", "aac", "-movflags", "+faststart", "-y", outputPath,
    ]);
    proc.on("close", (code) => code === 0 ? resolve() : reject(new Error(`FFmpeg hata: ${code}`)));
...
function concatFiles(filePaths, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    fs.writeFileSync(listPath, filePaths.map(f => `file '${f.replace(/\\/g, "/")}'`).join("\n"));
    const proc = spawn(ffmpegPath, ["-f", "concat", "-safe", "0", "-i", listPath, "-c", "copy", "-movflags", "+faststart", "-y", outputPath]);
    proc.on("close", (code) => {
      try { fs.unlinkSync(listPath); } catch (_) {}
      code === 0 ? resolve() : reject(new Error(`Birlestirme hatasi: ${code}`));
    });
    proc.on("error", reject);
  });
}
...
function cleanup(...paths) {
  for (const p of paths) {
    try { if (Array.isArray(p)) p.forEach(f => fs.unlinkSync(f)); else fs.unlinkSync(p); } catch (_) {}
  }
}
...
app.post("/api/info", async (req, res) => {
  const { url } = req.body;
  if (!url || !isValidYoutubeUrl(url)) return res.status(400).json({ error: "Gecersiz YouTube URL" });
  try {
...
app.post("/api/clip", async (req, res) => {
  ...
  if (!url || !isValidYoutubeUrl(url)) return res.status(400).json({ error: "Gecersiz YouTube URL" });
  if (!segments || !Array.isArray(segments) || segments.length === 0) return res.status(400).json({ error: "En az bir segment gerekli" });

  for (const seg of segments) {
    if (seg.start == null || seg.end == null || seg.start >= seg.end) return res.status(400).json({ error: "Gecersiz zaman araligi" });
    if (seg.end - seg.start > 600) return res.status(400).json({ error: "Her segment max 10 dakika" });
  }
  ...
  try {
    ...
    if (segments.length === 1) {
      ...
      cleanup(tempPath);
      return res.json({ success: true, filename, downloadUrl: `/api/download/${encodeURIComponent(filename)}` });
    }
    ...
    res.json({ success: true, filename, downloadUrl: `/api/download/${encodeURIComponent(filename)}` });
  } catch (err) {
...
app.get("/api/download/:filename", (req, res) => {
  ...
  if (filename.includes("..") || filename.includes("/") || filename.includes("\\")) return res.status(400).json({ error: "Gecersiz dosya adi" });
...
setInterval(() => {
  ...
  [CLIPS_DIR, TEMP_DIR].forEach(dir => {
    try { fs.readdirSync(dir).forEach(file => { const fp = path.join(dir, file); if (now - fs.statSync(fp).mtimeMs > 3600000) fs.unlinkSync(fp); }); } catch (_) {}
  });
}, 600000);
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log("");
  console.log("  ==============================");
  console.log("   YouTube Klip Sunucusu");
  console.log("  ==============================");
  console.log(`  Tarayici:  http://localhost:${PORT}`);
